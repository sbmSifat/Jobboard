...
function functionCompare($CvDetails,$getjobtype) {
    ...
    $stmtcv = $con->query("SELECT * FROM `cv`"); //running query
    ...
    class job{
        ...
        function __construct($actualSerial,$type, $loc,$salary,$overt,$skills,$sim,$name,$emailcv) {
...
//    }
        ...
        while($row = $stmtcv->fetch(PDO::FETCH_ASSOC)){
            ...
            //array_push($jobs,new job("get it","now"));
            array_push($jobs,new job($row["Serial"],$row["JobType"],$row["JobLocation"],$row["Salary"],$row["Overtime"],$row["Skills"],$result,$row["Name"],$row["Email"]));
            $count++;
            }
        }
